...
public class Bufet {
    ...
    public void listaMostrar() {
        for(Persona listaPersona : listaPersonas) {
            ...
            for (Pedido listaPedido : listaPedidos) {
                System.out.println( "tiempo.Fecha de creacion: " + listaPedido.getFechaCreacion() + " Horario de entrega: " + listaPedido.getHorarioEntrega());
                for (Plato listaPlato : listaPlatos) {
                    if(listaPersona instanceof Profesor) {
                        double descuentoAplicar = ((Profesor) listaPersona).getDescuento();
                        System.out.println("cocina.Plato: " + listaPlato.getNombrePlato() + " Precio: " + (listaPlato.getPrecioPlato() - (descuentoAplicar*listaPlato.getPrecioPlato()/100)));
                    } else {
                        System.out.println("cocina.Plato: " + listaPlato.getNombrePlato() + " Precio: " + listaPlato.getPrecioPlato());
                    }
